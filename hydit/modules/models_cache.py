...
class HunYuanDiT_cache(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def __init__(self,
        ...
        self.cache_at_branch = cache_at_branch
    ...
    def forward(self,
        ...
        cache_at_branch = self.cache_at_branch
        if self.cache_cont % cache_step == 0:
            for layer, block in enumerate(self.blocks):
                if layer == len(self.blocks) - cache_at_branch:
                    self.cache_val = x
            ...
            for layer, block in enumerate(self.blocks):
                if not (layer < cache_at_branch or layer >= len(self.blocks) - cache_at_branch):
                    continue
                if layer == len(self.blocks) - cache_at_branch:
                    x = self.cache_val
                if layer > self.depth // 2:
...
def DiT_g_2(args, **kwargs):
    return HunYuanDiT_cache(args, depth=40, hidden_size=1408, patch_size=2, num_heads=16, mlp_ratio=4.3637, **kwargs)


def DiT_XL_2(args, **kwargs):
    return HunYuanDiT_cache(args, depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
